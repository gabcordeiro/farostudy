/**
 * Invoca a edge function `generate-cards`. O SDK anexa o JWT do usuário,
 * então a chave do Gemini nunca toca o browser.
 */
...
export async function generateCards(input: AiGenerateInput): Promise<GenerateResult> {
  const payload = aiGenerateSchema.parse(input); // válida antes de enviar (#14)
